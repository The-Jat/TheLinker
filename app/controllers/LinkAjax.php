...
class LinkAjax extends Controller {
	...
	private function check_url($url) {
		if($url) {
			...
			$blacklisted_keywords = settings()->links->blacklisted_keywords;

			/* Convert to array if needed */
			if(!is_array($blacklisted_keywords)) {
    			$blacklisted_keywords = json_decode($blacklisted_keywords, true);

    			/* fallback if it's comma-separated string */
    			if(!is_array($blacklisted_keywords)) {
	        		$blacklisted_keywords = array_map('trim', explode(',', $blacklisted_keywords));
    			}
			}

			if(in_array(mb_strtolower($url), $blacklisted_keywords)) {
    			Response::json(l('link.error_message.blacklisted_keyword'), 'error');
			}
